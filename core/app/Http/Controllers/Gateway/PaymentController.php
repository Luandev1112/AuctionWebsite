...
class PaymentController extends Controller
{
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            notify($user, 'PRODUCT_PAYMENT_COMPLETE', [
            ...
            if($data->order_id){
                ...
                if($order){
                    ...
                    $transaction->amount = ($order->amount - $order->charge);
                    $transaction->post_balance = $productUser->balance;
                    $transaction->charge = $order->charge;
                    $transaction->trx_type = '+';
                    $transaction->details = 'Payment for ' . $order->order_number . ' number';
                    $transaction->trx = $data->trx;
                    $transaction->save();
